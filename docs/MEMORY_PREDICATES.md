...
- **Single-valued attributes.** Facts like `current_title` or `current_role` should supersede on new writes, not accumulate. The registry handles this automatically.
...
- **`singleValued`** (writes supersede prior): `current_title`, `current_role`.
